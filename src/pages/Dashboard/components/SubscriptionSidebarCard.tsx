    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="relative mx-3 my-3 group-data-[collapsible=icon]:hidden overflow-hidden rounded-2xl bg-gradient-to-br from-[#7c88c4] to-[#5b5ea6] shadow-[0_4px_20px_rgba(124,136,196,0.35)]">
      ...
      <div className="relative p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-1.5 bg-white/15 backdrop-blur-sm text-white/90 text-[11px] font-medium px-2.5 py-1 rounded-full">
            <Clock className="w-3 h-3" />
            <span>{t("dashboard.sidebar.subscriptionCard.remainingDays")} 14</span>
          </div>
          <button
        ...
        <div className="mb-3">
          ...
          <span className="text-[11px] text-white/60 mt-0.5 block">
            {t("dashboard.sidebar.subscriptionCard.subscribedTo")}
          </span>
        </div>

        {/* Activation Text */}
        <p className="text-yellow-300 text-sm leading-relaxed mb-4">
        ...
        <div className="w-full h-0.5 bg-white/20 rounded-full mb-4 overflow-hidden">
          <div className="h-full w-1/3 bg-yellow-300 rounded-full" />
        </div>

        {/* Contact Button */}
        <button
          onClick={handleContact}
          className="flex items-center gap-2 bg-white hover:bg-white/90 text-[#5b5ea6] text-sm font-semibold px-5 py-2.5 rounded-xl transition-all shadow-lg hover:shadow-xl active:scale-95 w-full justify-center"
        >
